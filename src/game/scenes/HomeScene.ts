...
export class HomeScene extends Phaser.Scene {
  ...
  private createTopBar(): void {
    const live = this.add.container(28, 42);
    ...
    createFlag(this, 333, 59, country.code, 46);
  }

  private createHero(): void {
    const halo = this.add.circle(195, 188, 104, COLORS.cyan, 0.035).setStrokeStyle(1, COLORS.cyan, 0.08);
    const ring = this.add.graphics().lineStyle(2, COLORS.lime, 0.3).arc(195, 188, 82, Phaser.Math.DegToRad(210), Phaser.Math.DegToRad(338));
    this.tweens.add({ targets: halo, scale: 1.12, alpha: 0.015, yoyo: true, repeat: -1, duration: 1800, ease: 'Sine.InOut' });
    this.add.text(195, 120, 'PENALTY', { fontFamily: DISPLAY_FONT, fontSize: '60px', fontStyle: 'bold italic', color: '#f7fbff', letterSpacing: -1.5 }).setOrigin(0.5);
    const rush = this.add.text(195, 174, 'RUSH', { fontFamily: DISPLAY_FONT, fontSize: '76px', fontStyle: 'bold italic', color: '#caff38', stroke: '#07110c', strokeThickness: 3, letterSpacing: -1 }).setOrigin(0.5);
    this.add.text(195, 224, '5 TIRS  •  30 SECONDES  •  UN RANG', { fontFamily: BODY_FONT, fontSize: '11px', fontStyle: 'bold', color: '#9bb0ba', letterSpacing: 0.8 }).setOrigin(0.5);
    this.tweens.add({ targets: rush, y: 171, yoyo: true, repeat: -1, duration: 1300, ease: 'Sine.InOut' });
    ring.setAlpha(0.8);
  }
  ...
  private createActions(): void {
    createButton(this, 195, 414, 'Jouer maintenant', () => { gameState.challengeMode = false; this.scene.start('CountrySelectScene'); }, { width: 338 });
  ...
  private createLeaderboardPreview(): void {
    this.add.text(28, 573, 'CLASSEMENT EXPRESS', { fontFamily: DISPLAY_FONT, fontSize: '18px', fontStyle: 'bold', color: '#f7fbff', letterSpacing: 0.7 });
    this.add.text(362, 578, 'VOIR TOUT', { fontFamily: BODY_FONT, fontSize: '9px', fontStyle: 'bold', color: '#35e8ff' }).setOrigin(1, 0).setInteractive({ useHandCursor: true }).on('pointerup', () => this.scene.start('LeaderboardScene'));
    const entries = [
      ['01', 'Striker94', 'BR', '11 980'], ['02', 'LeMur', 'FR', '10 650'], ['03', 'AtlasKick', 'MA', '9 720']
    ];
    entries.forEach((entry, index) => {
      const y = 626 + index * 58;
    ...
    this.add.text(195, 815, 'AUCUN COMPTE REQUIS  •  JOUE INSTANTANÉMENT', { fontFamily: BODY_FONT, fontSize: '8px', fontStyle: 'bold', color: '#536771', letterSpacing: 0.6 }).setOrigin(0.5);
  }
}
